...
docker run  --privileged --network host --rm  --env="DISPLAY" --env="QT_X11_NO_MITSHM=1" --env="ROS_DOMAIN_ID=$ROS_DOMAIN_ID" \
...
--user "$(id -u):$(id -g)" --name "dorothy_edge" dorothy2-edge:dev \
/bin/bash -c -i 'ros2 launch dorothy_robot dorothy_edge.launch.py "slam":="True"' 
